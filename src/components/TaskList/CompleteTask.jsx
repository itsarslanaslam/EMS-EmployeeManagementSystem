...
const CompleteTask = ({ data, onTaskUpdate }) => {
  const completedTasks = data.tasks?.filter(task => task.completed && !task.failed) || [];
    ...
    const updatedEmployees = employees.map(emp => {
      if (emp.email === data.email) {
        ...
        const updatedTaskCounts = {
          ...
          completed: Math.max(0, (emp.taskcounts?.completed || 0) - 1),
      ...
      <div className='space-y-4'>
        {completedTasks.length === 0 ? (
          ...
          completedTasks.map((task) => (
            <div key={task.id} className='bg-gradient-to-br from-green-900/50 to-green-800/30 border border-green-400/30 rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 backdrop-blur-sm relative overflow-hidden'>
              {/* Background pattern */}
              ...
              <div className='relative z-10'>
                ...
                <button
                  ...
                  className='w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-4 py-3 rounded-lg transition-all duration-200 font-medium shadow-md hover:shadow-lg transform hover:-translate-y-0.5'
                >
                  🔄 Reopen Task
                </button>
              </div>
